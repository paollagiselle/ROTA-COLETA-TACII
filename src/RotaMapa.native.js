...
//Código com mapa estático - sem rota em tempo real
/*import React from 'react';
import { StyleSheet, View } from 'react-native';
import MapView, { Marker, Polyline } from 'react-native-maps'; 

// Dados de Exemplo (Simulação de Coordenadas)
const pontosColeta = {
  A: { latitude: -22.9519, longitude: -43.2105, title: 'Início (Base)' }, // Ex: Base na Tijuca
  B: { latitude: -22.9550, longitude: -43.2150, title: 'Ponto B - Orgânico' },
  C: { latitude: -22.9480, longitude: -43.2050, title: 'Ponto C - Reciclável' },
  D: { latitude: -22.9580, longitude: -43.2000, title: 'Ponto D - Eletrônico' },
  E: { latitude: -22.9600, longitude: -43.2180, title: 'Ponto E - Orgânico' },
...
export default function RotaMapa() {
  const initialRegion = {
    latitude: -22.95, // Centro da visualização inicial
    longitude: -43.21,
  ...
  return (
    <View style={styles.container}>
      ...
      >
        {Object.keys(pontosColeta).map((key) => {
          ...
          return (
            <Marker
              ...
              coordinate={ponto}
              title={ponto.title}
              pinColor={key === 'A' ? '#0097e6' : '#2ecc71'} // Cor diferente para o ponto de partida
            />
          );
        })}
        ...
        <Polyline
          coordinates={rotaSimulada}
          strokeColor="#e65100" // Cor da rota (Laranja/Vermelho)
          strokeWidth={4}
        />
...
const styles = StyleSheet.create({
  container: {
    height: 300, // Altura fixa para caber na tela de extrato
    width: '100%',
    borderRadius: 12,
    overflow: 'hidden', // Importante para garantir que o mapa respeite o borderRadius
    shadowColor: "#000",
    shadowOpacity: 0.1,
    shadowOffset: { width: 0, height: 2 },
    shadowRadius: 5,
